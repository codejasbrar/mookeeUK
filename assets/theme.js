...
var roar = {
                ...
                p.each(function() {
                    if (!$(this).hasClass("has-swatch-finished")) {
                        ...
                        if ($(this).find(".single-option-selector-item").each(function() {
                            ...
                            var e = [];
							if ("1" == window.swatch_size && e.push("Size"), "1" == window.swatch_color && e.push("Filling"), e.push("filling"), "1" == window.swatch_color && (e.push("Color"), e.push("Colour"), e.push("color"), e.push("colour")), 0 < e.length) {
                                ...
                                for (i = 0; i < y.options.length; i++) {
                                    ...
                                    if (c = "object" == typeof y.options[i] ? y.options[i].name : y.options[i], b = !1, w = !1, -1 < e.indexOf(c)) {
                                        b = !0, C = i;
                                        var M = c.toLowerCase();
                                        if (/filling|color|colour|filling/i.test(M) && (w = !0), b) {
                                            var d = [];
                                            for (j = 0; j < y.variants.length; j++) {
                                                ...
                                                0 > d.indexOf(h) && ("color" != M && "colour" != M && "filling" != M ? (N = h, L = "btooltip") : "1" == window.swatch_color_advanced ? null !== v[f] && void 0 !== v[f] && "" != v[f] ? (L = "img btooltip swatch_color_advanced", N = "<i style=\"background-image: url(" + x + v[f] + ".png" + S + ")\"></i>") : null === m.featured_image ? N = "<i style=\"background-color:" + h + "; background-image: url(" + x + f + ".png" + S + ")\"></i>" : (L = "img btooltip swatch_color_advanced", N = "<i style=\"background-image: url(" + o(m.featured_image.src) + ")\"></i>") : N = "<i style=\"background-color:" + h + "; background-image: url(" + x + f + ".png" + S + ")\"></i>", z = $("#single-option-selector-" + y.id + "-" + C + "-" + u).val() == h ? "selected " : "", P = P + "<div class=\"swatch-element " + M + f + " available\"><input data-variant_id="+m.id+" data-id=\"#single-option-selector-" + y.id + "-" + C + "-" + u + "\" data-value=\"" + h + "\"  class=\"swatch-radio " + z + "\" id=\"swatch-single-option-selector-" + y.id + "-" + C + "-" + f + "-" + u + "\" type=\"radio\" data-swatch=\"" + M + "\" data-poption=\"" + C + "\" name=\"option-" + C + "\" value=\"" + h + "\"><label data-variant_id="+m.id+" for=\"swatch-single-option-selector-" + y.id + "-" + C + "-" + f + "-" + u + "\" class=\"" + L + "\" title=\"" + h + "\"><span class=\"soldout-image\"></span>" + N + "</label></div>", d.push(h))
                                            }
                            ...
                            0 < p.find(".wrapper-swatches-product-item").length && (g = p.find(".wrapper-swatches-product-item .swatch-radio"), g.unbind("click"), g.on("click", function() {
                              ...
                              // console.log("$(this).parents",$(this).parents(".product_grid_item").find(".product_option_link a"));
                              var href_var = $(this).parents(".product_grid_item").find(".product_option_link a").eq(0).attr("href").split("?");
                              console.log("href_var",href_var[0]);
                                $(this).parents(".product_grid_item").find(".product_option_link a").eq(0).attr("href",href_var[0]+"?variant="+variant_id);
        ...
        handleCompareEvent: function() {
            ...
            e.on("click", "a.add_to_compare", function() {
                ...
                null != r && (r = r.split(",")), r = jQuery.grep(r, function(e) {
                    return e != a
                }), r = $.trim(r), RoarCookie.cookie.rtwrite("rt-compare", r), $(".itg_" + a).remove(), 0 >= r.length && $(".mfp-close").trigger("click")
            })
        },
        ...
        mapPagination: function() {
            if ($(document.body).on("click", ".itg_ajax_load_button a", function(t) {
                    if (t.preventDefault(), $(".pagination a.next").length) {
                        $(".itg_ajax_load_button a").attr("data-processing", 1);
                        var e = $(".pagination a.next").attr("href"),
                            a = $(".itg_ajax_load_button a").attr("data-loading-items"),
                            o = $(".itg_ajax_load_button a").attr("data-no-more");
                        $(".itg_ajax_load_button").hide(), $(".pagination").before("<div class=\"itg_ajax_load_more_loader animated fadeIn\"><a href=\"#\"><img src=\"https://cdn.shopify.com/s/files/1/0255/4029/6738/files/page_loader.gif\" class=\"load_img\"></a></div>"), $.get(e, function(e) {
                            ...
                            }), roar.mapPaginationCallback(), $(".itg_ajax_load_more_loader").fadeOut("slow"), $(".itg_ajax_load_button").fadeIn("slow"), $(".itg_ajax_load_button a").attr("data-processing", 0), 0 == $(".pagination a.next").length && ($(".itg_ajax_load_button").addClass("finished").removeClass("itg_ajax_load_more_hidden"), $(".itg_ajax_load_button a").show().html(o).addClass("disabled"))
                        })
                    } else {
                        var o = $(".itg_ajax_load_button a").attr("data-no-more");
                        $(".itg_ajax_load_button").addClass("finished").removeClass("itg_ajax_load_more_hidden"), $(".itg_ajax_load_button a").show().html(o).addClass("disabled")
                    }
                }), $(".itg_ajax_load_button").hasClass("itg_ajax_load_more_hidden")) {
                Math.abs(0);
                $(window).scroll(function() {
                    if ($(".products").length) {
                        var e = $(".products").offset().top + $(".products").outerHeight(),
                            t = e - $(window).scrollTop();
                        t - 0 < $(window).height() && 0 == $(".itg_ajax_load_button a").attr("data-processing") && $(".itg_ajax_load_button a").trigger("click")
                    }
                })
            }
        },
...
theme.CurrencyPicker = function() {
    ...
    return e.prototype = _.assignIn({}, e.prototype, {
            ...
            if (1 == parseInt(window.rtl) && (a = !0), "left" == this.settings.product_design || "bottom" == this.settings.product_design || "compact2" == this.settings.product_design || "split" == this.settings.product_design || "sidebar" == this.settings.product_design || "simple" == this.settings.product_design || "full-screen" == this.settings.product_design) {
                if (0 < $(e.selectors.productThumbImages).length) {
                    var o = $(e.selectors.productThumbImages).find(".thumbnails"),
                        ...
                        n = 4,
                        s = !1;
                    if (4 < this.settings.product_image_count ? (n = 4, s = !0) : n = this.settings.product_image_count - 1, $(".product-page-section").hasClass("product-has-sidebar") && (3 < this.settings.product_image_count ? (n = 3, s = !0) : n = this.settings.product_image_count - 1), !0 == s) t.not(".slick-initialized").slick({
                        rtl: a,
                        slidesToShow: 1,
                        slidesToScroll: 1,
                        infinite: !1,
        ...
        _initStickyImages: function() {
            $("body").hasClass("itg-product-design-sticky") && $(".product-design-sticky .product-summary").stick_in_parent()
        },
